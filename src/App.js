...
function SessionCard({ session, index, onUpdate, onRemove, canRemove }) {
  ...
  const typeBtn = (selected) => ({
    ...
    border: `1.5px solid ${selected ? C.select : C.border}`,
    borderRadius: "4px",
    background: selected ? C.select : C.section,
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        {canRemove && (
          <button onClick={() => onRemove(session.id)} style={{
            padding: "6px 14px", background: "#fff5f5",
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {durations.map((d) => (
            <button key={d.value} onClick={() => onUpdate(session.id, "duration", d.value)} style={durBtn(session.duration === d.value)}>
              {d.label}
            </button>
          ))}
        </div>
  ...
  return (
    <>
      ...
      <div dir="rtl" style={{
        ...
        {/* البانر */}
        <div style={{
          width: "100%", background: C.primary, borderRadius: "4px",
          padding: "22px", marginBottom: 14, color: "white",
          boxShadow: "0 2px 10px rgba(84,108,75,0.25)",
        }}>
          <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 5, fontFamily: font }}>حاسبة الغياب</div>
          <div style={{ fontSize: 15, opacity: 0.85, fontFamily: font }}>اطلع على نسبة غياباتك باستمرار</div>
        </div>

        {/* الإعدادات العامة - قائمة منسدلة */}
                  ...
                  {["10", "15", "20", "25", "30"].map((v) => {
                    ...
                    return (
                      <button key={v} onClick={() => { setAllowedPercent(v); setResult(null); }} style={{
                        padding: "9px 16px",
                        border: `1.5px solid ${sel ? C.select : C.border}`,
                        borderRadius: "4px",
                        background: sel ? C.select : C.section,
            ...
            <div style={{
              ...
              <div style={{ borderTop: `1.5px dashed ${C.border}`, paddingTop: 16, marginBottom: 16 }}>
                ...
                {result.sessionDetails.map((s, i) => {
                  ...
                  return (
                    <div key={s.id} style={{
                      padding: "14px 16px", background: statusBg(ss),
                      borderRadius: "4px", marginBottom: 8,
                      border: `1.5px solid ${statusBorder(ss)}`,
                    }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                        <div>
                          <span style={{ fontSize: 15, fontWeight: 800, color: C.text, fontFamily: font }}>
                            لقاء {i + 1} — {s.type}
                          </span>
        ...
        <footer style={{
          ...
          marginTop: "auto", paddingTop: 14, paddingBottom: 24, textAlign: "center",
        }}>
